...
public class PlayerGunFireAbility : MonoBehaviour
{
    ...
    private IEnumerator Reload_Coroutine()
    {
        ...
        yield return new WaitForSeconds(CurrentGun.Relode);
        ...
        if (Input.GetMouseButton(0) && _timer >= CurrentGun.FireCoolTime && CurrentGun.BulletRemainCount > 0)
        {
            ...
            if (isHit)
            {
                ...
                if (hitObject != null)  // 때릴 수 있는 친구인가요?
                {
                    ...
                    DamageInfo damageInfo = new DamageInfo(DamgeType.Normal, CurrentGun.damage);
                    damageInfo.Poition = hitInfo.point;
                    damageInfo.Normal = hitInfo.normal;
                    if (UnityEngine.Random.Range(0, 5) == 0)
                    {
                        Debug.Log("크리티컬");
                        damageInfo.DamgeType = DamgeType.Critical;
                        damageInfo.Amount *= 2;
                    }
